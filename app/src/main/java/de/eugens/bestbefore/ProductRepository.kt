...
class ProductRepository {
    ...
    suspend fun analyzeImages(items: List<ScannedItem>): List<ExpirationInfo> = withContext(Dispatchers.IO) {
    ...
    suspend fun saveAnalysisResults(results: List<ExpirationInfo>, items: List<ScannedItem>) = withContext(Dispatchers.IO) {
        val currentUser = auth.currentUser
    ...
    private fun resizeAndEncodeBitmap(bitmap: Bitmap): String {
        val ratio = Math.min(
            Constants.BITMAP_MAX_WIDTH.toFloat() / bitmap.width,
            Constants.BITMAP_MAX_HEIGHT.toFloat() / bitmap.height
        )
